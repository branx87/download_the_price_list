...
class IekDownloader(BaseDownloader):
    ...
    def download(self, vendor: str) -> Path:
        ...
        temp_filename = f"IEK_price_{datetime.now().strftime('%Y%m%d_%H%M')}_temp.zip"
        temp_filepath = self.download_dir / temp_filename
        final_filename = f"IEK_price_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
        ...
        try:
            ...
            try:
                with zipfile.ZipFile(temp_filepath, 'r') as zip_ref:
                    ...
                    logger.info(f"✅ Файл загружен: {final_filename}")
